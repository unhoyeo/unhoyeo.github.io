...
그 중 가장 대표적인 것이 바로 **쿠키(Cookie)**와 **세션(Session)**이다.
...
세션은 "**중요한 정보는 모두 서버에 저장**하고, 클라이언트와는 **추정 불가능한 임의의 식별자(세션 ID)**로만 연결"하는 것을 의미한다.
...
- **getSession(false)**: 세션이 있으면 기존 세션을, 없으면 **null** 반환 → **세션 존재 여부(사용자 로그인 여부)**를 확인할 때 사용
