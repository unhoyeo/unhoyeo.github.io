...
- <strong>SELECT</strong>: 조회할 <strong>열(Column)</strong>을 지정 (무엇을 가져올 것인가?)
- <strong>FROM</strong>: 데이터가 들어있는 <strong>테이블(Table)</strong>을 지정 (어디에서 가져올 것인가?)
...
별칭에 공백과 같은 특수 문자를 포함하려면 <strong>백틱(`)</strong>으로 감싸주면 된다.
...
WHERE 절은 FROM 절 뒤에 위치하며, 테이블의 전체 데이터 중 <strong>특정 조건을 만족하는 행(Row)</strong>만 걸러내는 필터 역할을 한다.
