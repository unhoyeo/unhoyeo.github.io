...
**ViewResolver**는 논리 뷰 이름을 실제 렌더링 가능한 **물리 뷰 경로**로 변환하여 **실제 뷰 객체를 생성**하는 역할을 담당한다.

즉, **논리 뷰 이름을 받아서 실제 뷰 객체를 반환**하는 것이 ViewResolver의 핵심 역할이다.
...
| 1 | **BeanName**ViewResolver | 뷰 이름과 일치하는 스프링 빈을 탐색 |
| 2 | **InternalResource**ViewResolver | 뷰 이름에 prefix와 suffix를 붙여 JSP 경로 생성 |

DispatcherServlet은 등록된 ViewResolver들을 **순차적으로 탐색**하면서 논리 이름을 실제 뷰로 변환한다.
...
InternalResourceViewResolver는 **JSTL**(JSP Standard Tag Library)을 사용할 수 있는 환경이라면, InternalResourceView 대신 이를 상속받은 **JstlView**를 반환한다.
