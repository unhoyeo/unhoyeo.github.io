...
스프링 MVC에서 **HTTP 요청 파라미터**를 핸들러 메서드의 **파라미터에 바인딩**할 때 사용하는 애노테이션이다.
...
**서블릿 API**가 이들을 모두 ServletRequest.getParameter() 기반으로 **통합해서 조회**할 수 있도록 설계되어 있기 때문이다.
...
| --- | --- |
| **속성명** | **설명** |
| value (=name) | 바인딩할 요청 파라미터의 이름 (**생략 시 변수명과 동일**하게 처리됨) |
...
- 변수 타입이 **String**이고, 파라미터가 **빈 문자열**일 경우:
  - required 여부에 상관 없이 <strong>빈 문자열 ""</strong>이 바인딩됨 (예외 발생X)
- 변수 타입이 <strong>기본형(int, long 등)</strong>이고, 파라미터가 <strong>없거나 빈 문자열</strong>일 경우:
...
- **이름이 동일**한 여러 개의 파라미터를 List로 바인딩 가능
...
모든 요청 파라미터는 전부 **문자열(String)** 형태로 들어오기 때문에, 스프링 MVC는 이를 **파라미터의 타입으로 자동 변환**한다.

이때 내부적으로 ConversionService가 동작하고, Converter, Formatter, PropertyEditor 중 적절한 방식으로 변환된다.

다음 과정을 거치면 **사용자 정의 타입 변환**도 가능하다.
...
- ConversionService는 **스프링의 모든 타입 변환 작업**을 처리하는 핵심 컴포넌트다.
...
- 자동으로 StringToIpPortConverter가 동작하여 **IpPort 객체로 변환**됨
...
5. **RequestParamMethodArgumentResolver**
   ...
   - 내부적으로 <strong>request.getParameter()</strong>로 값을 꺼냄
