...
데이터베이스 **커넥션을 새로 만드는 과정**은 생각보다 복잡하고 비용이 많이 든다.
...
1. 애플리케이션이 DB 드라이버를 통해 **커넥션을 요청**한다.
2. DB 드라이버는 데이터베이스와 **TCP/IP** 네트워크 연결을 맺는다.
3. ID/PW 등 **인증 정보**를 전달하고, 데이터베이스 내부에서 **인증** 과정을 거친다.
...
- 사용이 끝나면 커넥션을 닫는(close) 대신, **풀에 반납**(return)하여 **재사용**한다.
...
이 덕분에 DriverManagerDataSource를 사용하다가 HikariDataSource로 **커넥션 풀 기술을 변경**하더라도,
