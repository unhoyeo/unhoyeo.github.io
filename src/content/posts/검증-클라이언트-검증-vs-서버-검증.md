...
웹 애플리케이션에서 검증은 애플리케이션의 견고함과 사용자 경험을 좌우하는 핵심 기능이다.
...
- API 스펙을 잘 정의해서 검증 실패 시 일관된 응답 구조를 제공하면 클라이언트와 협업 효율 증가
...
- 검증 과정에서 오류 발생 시, 다음과 같은 형태로 Map에 담아둔다.
  ...
  - **value = 오류 메시지**
- 오류가 하나라도 있으면, 오류를 담은 Map을 model에 추가하여 뷰에 전달한다.
...
- **타입 변환 실패(TypeMismatch) 처리 문제**
  - price 같은 숫자 필드에 문자열을 저장할 경우, TypeMismatchException이 발생함
  - 이 경우 **컨트롤러 메서드가 호출되기도 전에** 바인딩에 실패했기 때문에, 400 오류가 발생함
- **사용자 경험(UX) 문제**
  - 바인딩 예외가 발생하더라도, UX를 위해 사용자가 입력한 값은 그대로 유지해야 함
  - 그러나 바인딩 자체가 실패했기 때문에 **사용자가 입력한 값은 사라지게 됨**
  - 따라서 사용자가 입력한 값도 별도로 관리해야 함
