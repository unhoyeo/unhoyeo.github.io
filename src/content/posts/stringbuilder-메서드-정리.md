...
- start 부터 end - 1 까지의 문자열을 s로 변경한다.
- ✅ 시간 복잡도: O(N)
...
- StringBuilder 객체를 일반적인 String으로 변환한다.
- ✅ 시간 복잡도: O(N)
...
- 문자열을 여러 번 수정할 때 (+ 연산자 대신 append() 사용)
- 문자열을 뒤집거나 특정 부분만 변경할 때 (reverse(), setCharAt())
- 문자열을 동적으로 생성해야 할 때 (delete(), insert())
