...
우선, <strong>기본 키(PK)</strong>는 테이블 내에서 절대 중복될 수 없다. 하지만 <strong>외래 키(FK)</strong>는 여러 번 중복될 수 있다.
...
핵심 원리는 <strong>테이블 별칭(Alias)</strong>을 사용해 하나의 테이블을 두 개의 다른 가상 테이블처럼 다루는 것이다. (예: 직원(e)과 상사(m))
...
CROSS JOIN은 **ON 조건 없이** 한 테이블의 모든 행을 다른 테이블의 모든 행과 가능한 모든 조합으로 연결하는 조인이다.
