...
데이터베이스의 진정한 힘은 데이터를 단순히 저장하고 조회하는 것을 넘어, 저장된 데이터를 원하는 정보로 **가공**하고 **계산**하는 데 있다.
...
- **CHAR\_LENGTH() vs LENGTH()**:
  ...
  - LENGTH(): **바이트(byte) 수**를 반환한다. (UTF-8 환경에서 한글은 보통 **3바이트**를 차지한다.)
...
NULL을 **의미 있는 다른 값으로 대체**하기 위해 다음과 같은 함수를 사용한다.
...
- **COALESCE(표현식1, 표현식2, ...)**:
  - 나열된 표현식들 중 **처음으로 NULL이 아닌 값**을 반환한다.
