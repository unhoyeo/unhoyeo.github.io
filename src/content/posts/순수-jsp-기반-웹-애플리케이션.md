...
| **? 역할 분리** | **비즈니스 로직**과 **뷰**가 한 클래스에 존재 | **비즈니스 로직**과 **뷰**가 하나의 파일에 존재 |
...
UI 수정과 비즈니스 로직 수정은 **서로 다른 시점, 다른 목적**으로 발생한다.
...
- MVC 패턴은 **관심사의 분리(SoC, Separation of Concerns)**를 실현하며,
  - **가독성**
  - **유지보수성**
  - **확장성**
  - **협업 효율성**
  - **테스트 용이성** 등을 크게 향상시킨다.
- **변경 주기가 다른 코드**(UI와 비즈니스 로직)는 **완전히 분리**해서 유지보수성을 확보하자!
